...
class Picture extends CActiveRecord
{
	...
	public function rules()
	{
		return array(
			...
			array('extension_id, album_id, name, clients_name, path, src, comment, file_info, filename', 'safe'),
		);
	}
		...
		if(parent::beforeSave()) {
			if($this->isNewRecord) {
				if(isset($this->image)) {
					...
					$this->src = AlbumPictures::SAVEDIR . '/' . $this->album_id . '/' . $this->filename;
					$this->path = Yii::app()->controller->relesePath(Yii::getPathOfAlias('webroot') . $this->src);
	...
	protected function afterSave()
	{
		...
		if(isset($this->kohanaImageObject, $this->filename)) {
			...
			if ($side > 1000) {
				if ($side > 1920) {
					$this->savePicturePreview(1920, 1080);
				}
				$this->savePicturePreview(1000);
			}
